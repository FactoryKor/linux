...
# -*- coding: utf-8 -*-
"""
linux_diagnose.py — Linux OS read-only diagnostic CLI.
...
target uses Azure Monitor — on-premises, AWS, GCP, or any other environment:
  ...
  --source direct                   SSH로 대상 호스트에 직접 접속해 표준 리눅스
                                    명령/procfs만으로 실시간 수집. Azure Monitor
                                    가 전혀 없는 온프레미스·AWS·GCP 등 모든 환경에서 동작.
...
[direct 모드] SSH로 접속해 읽기 전용 명령만 실행(서버를 변경하지 않음)
  1. /proc/stat(2회 샘플링)   CPU 사용률
  2. /proc/meminfo            메모리 사용률
  3. df -P                    디스크 사용률(마운트별)
  4. journalctl -k / dmesg    OOM Killer 발생 탐지
  5. journalctl -p err..alert 오류 로그 건수(폴백: /var/log/syslog·messages grep)
  6. apt/yum/dnf              보안 업데이트 대기 건수(패키지 관리자 감지 후 조회)
...
def collect_syslog_errors(cred, workspace_id: str, computer: str, tw: TimeWindow) -> list[dict]:
    return _query(cred, workspace_id, _SYSLOG_ERR_KQL.format(computer=computer, time_filter=tw.kql_filter()), tw)


def collect_oom_events(cred, workspace_id: str, computer: str, tw: TimeWindow) -> Optional[dict]:
    rows = _query(cred, workspace_id, _SYSLOG_OOM_KQL.format(computer=computer, time_filter=tw.kql_filter()), tw)
...
# --------------------------------------------------------------------------- #
# Direct collection (SSH) — works anywhere: on-prem, AWS, GCP, any cloud.
# No Azure Monitor / Log Analytics dependency; reads procfs/journalctl/df only.
# --------------------------------------------------------------------------- #
def resolve_password(env_name: str) -> str:
    ...
    if pwd:
        return pwd
    if sys.stdin.isatty():
        return getpass.getpass(f"SSH password ({env_name} 환경변수 미설정 — 직접 입력): ")
    raise SystemExit(f"{env_name} 환경변수가 설정되지 않았습니다(비대화형 환경). "
                     f"비밀번호는 CLI 인자로 받지 않으므로 환경변수 또는 --ssh-key-file을 사용하세요.")
...
def connect_ssh(args):
    ...
    if args.ssh_known_hosts:
        client.load_host_keys(args.ssh_known_hosts)
    if args.ssh_insecure_auto_add_host_key:
        client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    else:
        client.set_missing_host_key_policy(paramiko.RejectPolicy())
    kwargs: dict[str, Any] = {"hostname": args.host, "port": args.ssh_port,
                             "username": args.ssh_user, "timeout": 10}
    if args.ssh_key_file:
        kwargs["key_filename"] = args.ssh_key_file
        if args.ssh_key_passphrase_env:
            passphrase = os.environ.get(args.ssh_key_passphrase_env)
            if passphrase:
                kwargs["passphrase"] = passphrase
    else:
        kwargs["password"] = resolve_password(args.ssh_password_env)
    client.connect(**kwargs)
    return client


def _ssh_run(client, cmd: str, timeout: int = 20) -> tuple[str, str, int]:
...
def parse_proc_stat_cpu(sample1: str, sample2: str) -> Optional[dict]:
    def _cpu_totals(text: str) -> Optional[tuple[int, int]]:
        ...
        return None
    a, b = _cpu_totals(sample1), _cpu_totals(sample2)
    if not a or not b:
        return None
    ...
    return {"avg": busy_pct, "max": busy_pct}
...
def parse_df(text: str) -> list[dict]:
    rows: list[dict] = []
    skip_fs = ("tmpfs", "devtmpfs", "overlay", "squashfs")
    for line in text.splitlines()[1:]:
        parts = line.split(maxsplit=5)
        if len(parts) < 6:
            continue
        fs, _blocks, _used, _avail, pct, mount = parts
        if fs in skip_fs or mount.startswith("/snap"):
            continue
        try:
            used_pct = float(pct.rstrip("%"))
        except ValueError:
            continue
        rows.append({"InstanceName": mount, "avg_val": used_pct, "max_val": used_pct})
    return rows


def parse_oom(text: str) -> Optional[dict]:
    lines = [l for l in text.splitlines() if l.strip()]
    if not lines:
        return None
    return {"count": len(lines), "sample": lines[0][:300]}


def parse_log_error_count(text: str) -> list[dict]:
    ...
    try:
        ...
        n = 0
    return [{"Facility": "n/a", "SeverityLevel": "err/crit/emerg/alert", "count_": n}]


def parse_count_as_security(text: str) -> list[dict]:
    lines = [l.strip() for l in text.splitlines() if l.strip()]
    if not lines:
        return []
    try:
...
def collect_cpu_direct(client) -> Optional[dict]:
    out, _err, code = _ssh_run(client, "cat /proc/stat; sleep 2; cat /proc/stat")
    if code != 0:
        return None
    cpu_lines = [l for l in out.splitlines() if l.startswith("cpu ")]
    if len(cpu_lines) < 2:
        return None
    return parse_proc_stat_cpu(cpu_lines[0], cpu_lines[1])


def collect_memory_direct(client) -> Optional[dict]:
    out, _err, code = _ssh_run(client, "cat /proc/meminfo")
    return parse_meminfo(out) if code == 0 else None


def collect_disk_direct(client) -> list[dict]:
    out, _err, code = _ssh_run(client, "df -P")
    return parse_df(out) if code == 0 else []


def collect_oom_direct(client) -> Optional[dict]:
    out, _err, _code = _ssh_run(client,
        "journalctl -k -q --no-pager --since '-24 hours' 2>/dev/null "
        "| grep -iE 'out of memory|oom-kill|killed process'; "
        "dmesg 2>/dev/null | grep -iE 'out of memory|oom-kill|killed process'")
    return parse_oom(out)


def collect_syslog_errors_direct(client, hours: int) -> list[dict]:
    out, _err, _code = _ssh_run(client,
        f"{{ journalctl -p err..alert -q --no-pager --since '-{hours} hours' 2>/dev/null "
        f"|| grep -iE 'error|critical' /var/log/syslog /var/log/messages 2>/dev/null; }} | wc -l")
    return parse_log_error_count(out)


def collect_update_direct(client) -> list[dict]:
    mgr_out, _err, _code = _ssh_run(client,
        "command -v apt-get >/dev/null 2>&1 && echo apt || "
        "(command -v dnf >/dev/null 2>&1 && echo dnf || "
        "(command -v yum >/dev/null 2>&1 && echo yum || echo none))")
    mgr = (mgr_out.strip().splitlines() or ["none"])[-1]
    if mgr == "apt":
        out, _e, _c = _ssh_run(client, "apt list --upgradable 2>/dev/null | grep -ic security")
        return parse_count_as_security(out)
    if mgr in ("dnf", "yum"):
        out, _e, _c = _ssh_run(client, f"{mgr} -q --security check-update 2>/dev/null "
                                       f"| grep -c '^[a-zA-Z0-9]'")
        return parse_count_as_security(out)
    return []
...
def evaluate_control_plane(report: Report) -> None:
    ...
    if report.vm_info.get("resource_type") == "arc_machine":
        ...
        if os_type and "linux" not in os_type.lower():
...
def evaluate(report: Report, data: dict, thr: dict) -> None:
    if report.collection_mode != "direct":
        # direct 모드는 SSH 연결 성공 자체가 연결성 확인이므로 build_report_direct()에서
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog=TOOL_NAME,
        description="Read-only Linux OS diagnostic tool. Recommended path: onboard the server to "
                    "Azure Arc, then use Log Analytics telemetry (default, --workspace-id + "
    ...
    # direct 모드(SSH)는 Azure Arc를 연결할 수 없는 예외적인 경우를 위한 숨은 옵션이다 —
    ...
    dm.add_argument("--ssh-port", type=int, default=22, help=argparse.SUPPRESS)
    dm.add_argument("--ssh-user", help=argparse.SUPPRESS)
    dm.add_argument("--ssh-password-env", default="LINUX_DIAGNOSE_SSH_PASSWORD", help=argparse.SUPPRESS)
    dm.add_argument("--ssh-key-file", default="", help=argparse.SUPPRESS)
    dm.add_argument("--ssh-key-passphrase-env", default="", help=argparse.SUPPRESS)
    dm.add_argument("--ssh-known-hosts", default="", help=argparse.SUPPRESS)
    dm.add_argument("--ssh-insecure-auto-add-host-key", action="store_true", help=argparse.SUPPRESS)
    ...
    out.add_argument("--save-snapshot", default="",
                     help="이번 실행 결과(JSON)를 이 경로에 JSON Lines로 추가 저장합니다 — "
                          "주기 실행(cron/작업 스케줄러)과 결합하면 직접 시계열 이력을 축적할 수 있습니다.")
...
def build_report_azure_monitor(args, thr: dict) -> Report:
    ...
    if cred is not None and args.workspace_id:
        ...
        try:
            data["log_errors"] = collect_syslog_errors(cred, args.workspace_id, args.computer, tw)
        except Exception as e:  # noqa: BLE001
            report.add("syslog", "info", "Syslog 조회 실패", str(e))
        try:
            data["oom"] = collect_oom_events(cred, args.workspace_id, args.computer, tw)
        except Exception as e:  # noqa: BLE001
...
def build_report_direct(args, thr: dict) -> Report:
    ...
    if not args.ssh_user:
        raise SystemExit("--ssh-user 는 필수입니다(--source direct)")
    ...
    if args.ssh_insecure_auto_add_host_key:
        report.add("security", "warning", "SSH 호스트 키 검증 비활성화",
                   "--ssh-insecure-auto-add-host-key 로 실행되어 알 수 없는 호스트 키를 자동 "
                   "신뢰합니다. 중간자 공격(MITM)에 취약할 수 있습니다.",
                   recommendation="가능하면 --ssh-known-hosts 로 known_hosts 파일을 지정해 "
                                  "호스트 키를 명시적으로 검증하세요.")

    try:
        client = connect_ssh(args)
    except Exception as e:  # noqa: BLE001
        report.add("connection", "critical", "SSH 연결 실패", str(e),
                  recommendation="호스트/포트/자격 증명/방화벽/호스트 키를 확인하세요.")
        evaluate(report, {}, thr)
        return report
    report.sources.append("ssh")
    report.add("connectivity", "ok", "SSH 연결 정상", f"{host}:{args.ssh_port} 에 연결했습니다.")

    data: dict[str, Any] = {}
    try:
        with client:
            steps = [("cpu", lambda: collect_cpu_direct(client)),
                    ("memory", lambda: collect_memory_direct(client)),
                    ("disk", lambda: collect_disk_direct(client)),
                    ("oom", lambda: collect_oom_direct(client)),
                    ("log_errors", lambda: collect_syslog_errors_direct(client, args.hours))]
            if args.skip_patch_check:
                report.add("patch", "info", "패치 점검 생략",
                          "--skip-patch-check 지정으로 생략했습니다(패키지 저장소 네트워크 부하 방지).")
            else:
                steps.append(("update", lambda: collect_update_direct(client)))
            for key, fn in steps:
                try:
                    data[key] = fn()
                except Exception as e:  # noqa: BLE001
        ...
        report.add("connection", "warning", "일부 조회 중 연결 문제", str(e))
...
def build_report(args) -> Report:
    ...
    if args.source == "direct" and (args.start_time or args.end_time):
        raise SystemExit("--start-time/--end-time 은 direct 모드에서는 지원되지 않습니다 "
                         "(SSH는 대상 서버의 현재 시점 상태만 조회할 수 있습니다).")
